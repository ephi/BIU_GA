...
class TSPChromosome(GAFrameWork.Chromosome):
    ...
    def calc_distance(self, city_1_index_in_ch, city_2_index_in_ch):
        ...
        return np.linalg.norm(self.all_cities_arr[self.cities_indexed_arr[city_1_index_in_ch]] - self.all_cities_arr[self.cities_indexed_arr[city_2_index_in_ch]])
        ...
        for index in indexes:
            if index == 0:
                ...
                self.distance_from_first_city[index] = self.distance_from_first_city[index-1] + \
                    self.all_distances[self.cities_indexed_arr[index], self.cities_indexed_arr[index -1]]
    ...
    def update_distances_after_range_changed(self, range_changed : np.ndarray):
        ...
        if range_changed[1] + 1 < self.chromosome_size - 1:
            orig_distance_from_first_city_of_the_city_after_the_range = self.distance_from_first_city[range_changed[1] + 1]
        self.recalculate_distance_from_first_city(range(range_changed[0], min(range_changed[1] + 2, self.chromosome_size)))
        if range_changed[1] + 1 < self.chromosome_size - 1:
            new_distance_from_first_city_of_the_city_after_the_range = self.distance_from_first_city[range_changed[1] + 1]
    ...
    def swap_ranges(self, self_range : np.ndarray, other_chromosome, other_range : np.ndarray, orig_self, orig_other) -> np.bool8:
        ...
        temp_sliced_self_indexes_vector, temp_sliced_other_indexes_vector = self.fix_ranges_to_have_same_indexes(temp_sliced_self_indexes_vector, temp_sliced_other_indexes_vector)
    ...
    def swap_two_cities(self, indexes_to_swap):
        ...
        if indexes_to_swap[1] - indexes_to_swap[0] == 1 :
             self.recalculate_distance_from_first_city(np.linspace(start = indexes_to_swap[0], stop = min(indexes_to_swap[1] + 1, self.chromosome_size-1), num = 3, dtype = np.uint8))
        else:
            for index in indexes_to_swap:
                self.recalculate_distance_from_first_city(np.linspace(start = index, stop = min(index + 1, self.chromosome_size-1), num = 2, dtype = np.uint8))
...
def tsp_crossover(chromosome_1 : TSPChromosome, chromosome_2 : TSPChromosome, generation : int):
    ...
    range_length_to_swap = np.random.randint(2, 2 + ceil((chromosome_size/2 - 2) * ((NUM_OF_GENERATIONS - generation) / NUM_OF_GENERATIONS)), dtype=np.uint8)
    ...
    while need_to_be_splited:
        ...
        swap_ranges_between_two_chromosomes(new_child_1, new_child_2, range_start_1, range_start_2, current_length_from_start, chromosome_1, chromosome_2)
    ...
    if range_start_1 != range_end_1:
        swap_ranges_between_two_chromosomes(new_child_1, new_child_2, range_start_1, range_start_2, range_length_to_swap, chromosome_1, chromosome_2)
...
if __name__ == '__main__':
    ...
    randomized_chromosomes = [TSPChromosome(all_cities_arr=all_cities_arr, all_distances=all_distances).randomize_value() for i in range(NUM_OF_CHROMOSOMES)]
